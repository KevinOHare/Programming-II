...
public class Queue {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		while (!sort.isEmpty()) {
			...
			// with patients
			if (treat.getFirst().getCountTimer() == 10) {
				treat.getFirst().setAvailable(true);
			}
			if (treat.get(1).getCountTimer() == 10) {
				treat.get(1).setAvailable(true);
			}
			if (treat.get(2).getCountTimer() == 10) {
				treat.get(2).setAvailable(true);
			}
			if (treat.get(3).getCountTimer() == 10) {
				treat.get(3).setAvailable(true);
			}
			if (treat.get(4).getCountTimer() == 10) {
				treat.get(4).setAvailable(true);
			}
			if (treat.getLast().getCountTimer() == 10) {
